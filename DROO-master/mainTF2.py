...
#    rolling_intv = 20
    ...
    # Access the x and y values from the plot object
    ...
    np.savetxt('data.txt', data, delimiter='\t', header='x\ty')
...
if __name__ == "__main__":
    ...
    # Calculate the average computation time per iteration
    avg_time = total_time / n

    # Print the average computation time
    print("Average computation time per iteration: {:.2f} seconds".format(avg_time))

    # Output the average computation time to a file
    with open('avg_time_script_1.txt', 'w') as f:
        f.write(str(avg_time))

    # Output the computation time of each iteration to a file
    with open('time_his_script_2.txt', 'w') as f:
        f.write(str(time) + '\n')
